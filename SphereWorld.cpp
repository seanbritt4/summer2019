...
/*
mid -- initial midpoint displacement
pert -- displacement maximum
smoothfact -- smoothing factor
H -- Power used in determining std dev of random variables. Range [1,2], smaller numbers = larger deviations
*/
void SphereWorld::initTileColors(GLfloat mid, GLfloat pert, GLint smoothfact, GLfloat H)
{
    int pos = ceil(log(lonDivisions*latDivisions));
    int grid = pow(2, pos);
...
//    }
    ...
    glm::vec3 arr[latDivisions*lonDivisions];
    int index = 0;
    for(unsigned int i = 0; i < latDivisions; i++)
        for(unsigned int j = 0; j < lonDivisions; j++){
            //if(ground[i][j] > 1 || ground[i][j] < 0)
            //    std::cout << i << " " << j << "\t" << ground[i][j] << std::endl;
            arr[indexColorArray(j,i)] = colorGradient(ground[i][j]);
        }

   // std::cout << "Fucking Hell" << std::endl;
    changeColorArray(arr);

}

double SphereWorld::randf()
{
    return (double)rand()/RAND_MAX;
}

double SphereWorld::randGauss(double m, double s)
{
    double x1, x2, w, y1;
    static double y2;
...
int SphereWorld::indexColorArray(int i, int j)
{
    return i*latDivisions + j;
}
